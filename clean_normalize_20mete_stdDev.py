...
for key, value in inputLayerCopies.items() :
    ...
    for key, value in arrayOfArraysToCleanIDs.items():
...
for key, value in inputLayerCopies.items():
     ...
     sumStringSub = ""
     for charact in sumStringSplit[1] :
        if charact.isdigit() or (charact == '.') :
            sumStringSub = sumStringSub + charact
     
     sum = float(sumStringSub)
     curLayer = inputLayerCopies[key]
     curLayer.startEditing()
     curLayer.addAttribute(QgsField("Normed_DrY", QVariant.Double, "double", 10, 10, "Yield as percentage of total yield", QVariant.Invalid))
     curLayer.updateFields()
     curLayer.commitChanges()
     print("Sum of total yield vol: " + str(sum))
     feats = curLayer.getFeatures()
     curLayer.startEditing()
     for feat in feats:
         feat["Normed_DrY"]= feat['Yld_Vol_Dr'] / sum
...
for key, value in inputLayerCopies.items():
    ...
    processing.run("qgis:statisticsbycategories", {'INPUT': tempFolder +'/INTERSECT' + key + '.shp','VALUES_FIELD_NAME':'Normed_DrY','CATEGORIES_FIELD_NAME':['id'], 'PREFIX': 'Yld_', 'OUTPUT': tempFolder + '/STATS_YIELD' + key})
    #Stats on _Moisture
    processing.run("qgis:statisticsbycategories", {'INPUT': tempFolder +'/INTERSECT' + key + '.shp','VALUES_FIELD_NAME':'Moisture__','CATEGORIES_FIELD_NAME':['id'],'OUTPUT': tempFolder + '/STATS_MOIST' + key})
#Once the stats are calculated, we will join both stats tables together on "id"
for key, value in inputLayerCopies.items():
    processing.run("native:joinattributestable", {'INPUT': tempFolder + '/STATS_YIELD' + key +'.gpkg','FIELD':'id','INPUT_2': tempFolder + '/STATS_MOIST' + key + '.gpkg' ,'FIELD_2':'id','FIELDS_TO_COPY':[],'METHOD':1,'DISCARD_NONMATCHING':False,'PREFIX':'Moist_','OUTPUT':tempFolder + '/JOINED_STATS' + key + '.gpkg' })
...
if( len(inputLayerCopies) != 0 ):
    ...
    for field in fields :
        if field.name() == "path" :
            curLayer.startEditing()
            i = curLayer.dataProvider().fieldNameIndex('path')
            curLayer.deleteAttribute(i)
        elif field.name() == "layer" :
            curLayer.startEditing()
            i = curLayer.dataProvider().fieldNameIndex('layer')
            curLayer.deleteAttribute(i)
        elif field.name() == "fid_2" :
            curLayer.startEditing()
